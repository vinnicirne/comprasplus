/**
 * admobService.js - Gerenciador Oficial de Monetização Google AdMob & Google Ads
 * Código 100% autêntico do repositório original (6bb692a) com suporte a ES Modules e Capacitor.
 */
...
export class AdMobManager {
  ...
  async init() {
    ...
    // 2. Carrega SDK Web em produção se aplicável
    if (!this.config.isTestMode) {
      this.loadWebAdsSdk();
    }

    // 3. Renderiza banners na interface visual (Web/PWA ou fallback visual)
  ...
  renderWebBanner() {
    ...
    slot.innerHTML = `
      ...
      <div class="admob-inner-content" style="cursor: pointer; display: flex; align-items: center; gap: 0.75rem; padding: 0.5rem 0.75rem;" onclick="window.admobManager.showInterstitial()">
        ...
        <button type="button" class="btn btn-primary btn-sm" style="font-size: 0.72rem; padding: 0.35rem 0.65rem; font-weight: 700; white-space: nowrap; border-radius: 6px; background: #006948; color: #fff; border: none; cursor: pointer;">
          CONFERIR
        </button>
      </div>
    `;
  ...
  /**
   * Renderiza bloco Nativo Avançado simulado ou inline
   */
  renderNativeAd(containerId = 'admob-native-slot') {
    ...
    container.innerHTML = `
      <div class="admob-native-container" style="background: var(--bg-card, #fff); border: 1px solid var(--border-color, rgba(188, 202, 192, 0.4)); border-radius: var(--radius-md, 14px); padding: 0.85rem; position: relative;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
          ...
          <span style="font-size: 0.72rem; color: #f59e0b; font-weight: 700;">
            ★★★★★ 4.9 <span style="color: var(--text-muted, #3d4a42); font-weight: 400;">(24k)</span>
          </span>
        </div>
        <div style="display: flex; align-items: flex-start; gap: 0.75rem;">
          ...
          <div style="flex: 1; min-width: 0;">
            <strong style="font-size: 0.92rem; color: var(--text-main, #131b2e); display: block; font-weight: 700;">
              Cartão Mercado Fácil com Cashback
            </strong>
            <p style="margin: 0.25rem 0 0.65rem; font-size: 0.78rem; color: var(--text-muted, #3d4a42); line-height: 1.35;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center;">
              ...
              <button type="button" class="btn btn-primary btn-sm" style="font-size: 0.75rem; padding: 0.35rem 0.85rem; font-weight: 700; background: #006948; color: #fff; border: none; border-radius: 6px; cursor: pointer;" onclick="window.admobManager.showInterstitial()">
                SOLICITAR
              </button>
            </div>
          </div>
      ...
      <div class="admob-overlay-card" style="max-width: 360px; border-radius: 16px; overflow: hidden; box-shadow: 0 20px 40px rgba(0,0,0,0.35);">
        ...
        <div class="admob-overlay-body" style="padding: 1.5rem 1rem; text-align: center;">
          <div class="admob-ad-preview-box">
            ...
            <button type="button" class="btn btn-primary" style="width: 100%; padding: 0.65rem; font-weight: 700; background: #006948; color: #fff; border: none; border-radius: 8px; cursor: pointer;" onclick="document.getElementById('btn-close-overlay-ad').click()">
              CONTINUAR NO APP
            </button>
          </div>
        </div>
...
// Instância global única do AdMob
export const admobManager = new AdMobManager();
export const admobService = admobManager;

window.admobManager = admobManager;
window.admobService = admobManager;
